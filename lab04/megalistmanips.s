...
mapLoop:
    lw t1, 0(s0)      # load the address of the array of current node into t1, Mistake: 1
    lw t2, 4(s0)        # load the size of the node's array into t2

    slli t3, t0, 2      # Mistake: 2
    add t1, t1, t3      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0
    
    addi sp, sp, -4     # Mistake 6
    sw t1, 0(sp)
    ...
    lw a0, 8(s0)        # load the address of the next node into a0, Mistake: 3
    mv a1, s1        # put the address of the function back into a1 to prepare for the recursion, Mistake: 4
